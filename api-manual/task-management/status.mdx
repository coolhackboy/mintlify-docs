...
Query image, video, audio, and 3D generation tasks and retrieve all returned files when complete.
...
This endpoint currently retrieves tasks by `task_id` without requiring an Authorization header or checking task ownership. The examples include a Bearer header so they can share the same client configuration as authenticated submit requests.
...
```json
{
  ...
  "data": {
    ...
    "files": [
      {
        "file_url": "https://file.goseaapp.com/images/image-abc123.jpg",
        "file_type": "image"
      }
    ],
...
| `data.files[].file_type` | string | Artifact type: `image`, `video`, `audio`, `lyrics`, `3d`, or `other` |
| `data.files[].watermark_url` | string | Optional watermarked variant |
| `data.files[].label` | string | Optional artifact role, such as model, preview, or timestamps |
| `data.files[].format` | string | Optional file format |
| `data.files[].content_type` | string | Optional MIME type |
| `data.files[].file_name` | string | Optional file name |
| `data.files[].file_size` | integer | Optional size in bytes |
| `data.files[].metadata` | object | Optional additional artifact metadata |
| `data.created_time` | string | ISO 8601 timestamp of task creation |
| `data.progress` | integer or null | Completion percentage when supplied by the service; some tasks may remain at 0 until completion |
| `data.error_message` | string or null | Error description when available; null when there is no error |
...
```json
{
  ...
  "data": {
    ...
    "files": [
      {
        "file_url": "https://file.goseaapp.com/videos/video-xyz789.mp4",
        "file_type": "video"
      }
    ],
    "created_time": "2025-11-12T10:30:00",
...
<Warning>
  **Content Validity**: File expiry depends on the storage provider. Download and save outputs that you need to retain.
</Warning>

<Info>
  **Credits**: Credits are deducted when the task is submitted. If the task fails, those credits are refunded automatically.
</Info>
...
<CardGroup cols={2}>
  <Card title="Submit Task" icon="upload" href="/api-manual/quickstart">
...
{/* task-workflows-20260910:start */}
## Additional generated files

Audio and 3D workflows use the same files array. file_type may be audio, 3d, image, or other. The array retains model assets, previews, textures, animation files, and timestamp files. Each item may also include label, format, content_type, file_name, file_size, and metadata. Process every returned file; metadata fields are optional.

{/* task-workflows-20260910:end */}
