...
The `resolution` parameter controls the effective output resolution.

| Resolution | Description |
| --- | --- |
| `1K` | Standard resolution |
| `2K` | High resolution |
| `4K` | Ultra-high resolution for true 4K-supported sizes |

### Billing Factors

Each request generates one image. Billing depends on `quality` and the effective output resolution determined by `size` and `resolution`. The rules below explain how the effective resolution is selected.
