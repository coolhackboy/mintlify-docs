...
| Output tier | USD per output image | Credits per output image |
| --- | ---: | ---: |
| Low / 1K | $0.040 | 8 |
| Medium / 1K | $0.060 | 12 |
| Low / 2K | $0.060 | 12 |
...
One credit equals $0.005. The default Medium / 1K tier costs 12 credits per output image.

Each input image adds **2 credits ($0.010)** per request. Total credits = output count (`n`) × output tier rate + input image count × 2. The input surcharge is not multiplied by `n`.

For example, Medium / 2K with `n: 4` and three input images costs **70 credits ($0.350)**.
