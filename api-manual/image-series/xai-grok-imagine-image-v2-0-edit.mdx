...
Billing depends on output quality (`quality`), output resolution (`resolution`), and output image count (`n`). Input image count (`image_urls`) adds a separate charge once per request; this charge is not multiplied by `n`.
